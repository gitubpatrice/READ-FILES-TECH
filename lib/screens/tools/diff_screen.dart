...
class _DiffScreenState extends State<DiffScreen> {
  ...
  /// Au-delà de cette taille, le diff LCS (O(n²) mémoire et temps) devient
  /// trop coûteux. On tronque et on prévient l'utilisateur.
  static const _maxLines = 500;
  bool _truncated = false;
  int _origLinesA = 0, _origLinesB = 0;
  ...
  Future<void> _compute() async {
    ...
    _origLinesA = linesA.length;
    _origLinesB = linesB.length;
    _truncated = linesA.length > _maxLines || linesB.length > _maxLines;
    final clippedA = linesA.length > _maxLines
        ? linesA.sublist(0, _maxLines)
        : linesA;
    final clippedB = linesB.length > _maxLines
        ? linesB.sublist(0, _maxLines)
        : linesB;
    final diff = _computeDiff(clippedA, clippedB);
    setState(() {
      _diff = diff;
  ...
  List<String> _lcs(List<String> a, List<String> b) {
    // a et b ont déjà été tronqués à _maxLines lignes par _compute().
    final maxA = a.length;
    final maxB = b.length;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Sélectionnez deux fichiers texte pour voir leurs différences',
              style: Theme.of(
                context,
            ...
            _pickBtn(
              _nameA ?? 'Choisir le fichier A',
              Colors.red.shade700,
              () => _pickFile(true),
            ),
            const SizedBox(height: 12),
            _pickBtn(
              _nameB ?? 'Choisir le fichier B',
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Diff tronqué aux $_maxLines premières lignes '
                    '(A : $_origLinesA, B : $_origLinesB). '
                    'Au-delà, l\'algorithme LCS devient trop lourd.',
